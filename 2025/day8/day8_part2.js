...
file.on('close', () => {
  ...
  for (var i = 0; i < distances.length; ++i) {
    ...
    if (nodeASet == nodeBSet) {
      ...
      // if a now contains all nodes, end
      if (
        (nodeASet >= 0 && circuits[nodeASet].size == nodes.length) ||
        (nodeBSet >= 0 && circuits[nodeBSet].size == nodes.length)
      ) {
        // node a and node b are our final nodes
        console.log(nodes[connection.nodeA].x * nodes[connection.nodeB].x);
        break;
      }
    }
  }
