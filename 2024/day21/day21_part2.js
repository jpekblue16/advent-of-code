...
const file = readline.createInterface({
  input: fs.createReadStream('day21_test.txt'),
...
file.on('line', (line) => {
  ...
  // get keypad commands
  var result = getCommands(line, 'num');

  // 25 layers of keyboards
  for (var i = 0; i < 25; ++i) {
    result = getCommands(result, 'arrow');
  }

  var codeValue = parseInt(line.substring(0, line.length - 1));

  total += codeValue * result.length;
});
